...
namespace MyVegasBot.Slots.Excalibur
{
    class Excalibur : Form1
    {
        ...
        public void Start()
        {
            ...
            while (true)
            {
                s = ScreenCapture.Shot(browser);
                Log("Checking for possible bonus games...");
                Search(s);
        ...
        void Search(Bitmap screen)
        {
            foreach(var image in images)
            {
                ...
                Log(string.Format("Searching for {0}...", image.Key));
                if (checker[0] != 0 || checker[1] != 0)
                {
                    Log(string.Format("Found {0}...", image.Key));
